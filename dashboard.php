...
<head>
  ...
  <style>
    ...
    .calendar-day {
      height: 90px; background: white; border-radius: 12px; padding: 6px;
      box-shadow: 0 3px 6px rgba(0,0,0,0.05); border: 1px solid #f4b41a50; cursor: pointer; transition: 0.2s;
    }
    .calendar-day:hover { background: #f4b41a20; }
    .calendar-day-number { font-weight: 600; color: #222; }
    .event-tag { background: #f4b41a; color: #222; padding: 2px 6px; border-radius: 6px; font-size: 10px; margin-top: 4px; display: inline-block; }
      ...
      <div id="customCalendar" class="custom-calendar w-full">
        <div class="flex justify-between items-center mb-6">
          <div class="flex gap-3">
            ...
            <select id="yearSelect" class="px-4 py-2 border border-gray-300 rounded-lg bg-white text-[#222] font-medium focus:outline-none focus:ring-2 focus:ring-[#f4b41a]">
            </select>
            <select id="themeSelect" class="px-4 py-2 border border-gray-300 rounded-lg bg-white text-[#222] font-medium focus:outline-none focus:ring-2 focus:ring-[#f4b41a]">
              <option value="white">White</option>
              <option value="pink">Light Pink</option>
              <option value="purple">Light Purple</option>
              <option value="orange">Light Orange</option>
              <option value="green">Light Green</option>
            </select>
          </div>
          <div class="flex gap-2">
            <button id="prevMonth" class="w-10 h-10 flex items-center justify-center border border-gray-300 rounded-lg bg-white hover:bg-gray-100 text-[#222] font-bold text-xl transition">&lt;</button>
            <button id="nextMonth" class="w-10 h-10 flex items-center justify-center border border-gray-300 rounded-lg bg-white hover:bg-gray-100 text-[#222] font-bold text-xl transition">&gt;</button>
          </div>
        </div>
...
<div id="eventDetailModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center hidden z-50 p-4">
  <div class="bg-white rounded-2xl shadow-xl relative w-full max-w-md p-8">
    ...
    <h2 id="eventDetailDate" class="text-2xl font-bold text-[#222] mb-4">Monday, December 15, 2025</h2>
    <div class="mb-6">
      <span id="eventStatusBadge" class="inline-block px-4 py-1 rounded-full text-sm font-medium text-white mb-3">UPCOMING</span>
      <h3 id="eventDetailTitle" class="text-xl font-bold text-[#222] mb-2">Conference</h3>
      <p id="eventDetailDescription" class="text-gray-600">No description</p>
    </div>
  </div>
</div>
...
// SAVE & CONTINUE (backend preserved)
document.addEventListener('DOMContentLoaded', function() {
...
<script>
  ...
  let events = [
    { date: '2025-10-16', name: 'Birthday Trip', status: 'successful', description: 'No description' },
    { date: '2025-10-18', name: 'Family Reunion', status: 'cancelled', description: 'No description' },
    { date: '2025-12-15', name: 'Conference', status: 'upcoming', description: 'No description' }
  ];

  const themeColors = { white:'#ffffff', pink:'#fce7f3', purple:'#f3e8ff', orange:'#fed7aa', green:'#d1fae5' };
  const themeBorders = { white:'#e5e7eb', pink:'#f9a8d4', purple:'#d8b4fe', orange:'#fb923c', green:'#6ee7b7' };
  ...
  function renderCalendar() {
    ...
    customCalendar.style.backgroundColor = themeColors[theme];
    customCalendar.style.borderColor = themeBorders[theme];
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      if (eventFound) {
        ...
        if (eventFound.status === 'upcoming') { tag.style.backgroundColor = '#f4b41a'; tag.style.color = '#222'; }
        else if (eventFound.status === 'cancelled') { tag.style.backgroundColor = '#ef4444'; tag.style.color = '#fff'; }
        else if (eventFound.status === 'successful') { tag.style.backgroundColor = '#10b981'; tag.style.color = '#fff'; }

        tag.innerText = eventFound.name;
        tag.style.cursor = 'pointer';
        tag.addEventListener('click', (e) => { e.stopPropagation(); showEventDetail(eventFound, dateString); });
  ...
  function showEventDetail(event, dateString) {
    ...
    if (event.status === 'upcoming') { badge.style.backgroundColor = '#f4b41a'; badge.style.color = '#222'; }
    else if (event.status === 'cancelled') { badge.style.backgroundColor = '#ef4444'; badge.style.color = '#fff'; }
    else if (event.status === 'successful') { badge.style.backgroundColor = '#10b981'; badge.style.color = '#fff'; }
  ...
  document.getElementById('prevMonth').addEventListener('click', () => {
    let m = parseInt(monthSelect.value), y = parseInt(yearSelect.value); m--; if (m < 0) { m = 11; y--; }
    monthSelect.value = m; yearSelect.value = y; renderCalendar();
  });
  document.getElementById('nextMonth').addEventListener('click', () => {
    let m = parseInt(monthSelect.value), y = parseInt(yearSelect.value); m++; if (m > 11) { m = 0; y++; }
    monthSelect.value = m; yearSelect.value = y; renderCalendar();
  });
